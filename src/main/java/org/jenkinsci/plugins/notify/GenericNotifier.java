...
public class GenericNotifier extends AbstractNotifier
{
    ...
    public GenericNotifier ( String notifyUrl, String notifyOn, String notifyTemplate) {
        ...
        throws InterruptedException, IOException
    {
    	...
    	if ( isIntermediateStep || TextUtils.isBlank ( notifyUrl ) ) {
            listener.getLogger().println( String.format( "Skipping notification as configured url is empty"));
            return;
        }
        if ( ! isNotifyableResult ) {
            listener.getLogger().println( String.format( "Skipping notification as build result level is configured to %s", notifyOn));
            return;
        }
        ...
        listener.getLogger().println( String.format( "Notifying as build result level %s is matching notification level %s", build.getResult(), notifyOn ));
        ...
        listener.getLogger().println( String.format( "Notification was SUCCESSFUL" ));
    }
